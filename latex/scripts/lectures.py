...
class Lecture():
    ...
    def edit(self):
       ...
       #     "x-terminal-emulator",
       #     "-e", "zsh", "-i", "-c",
       #     f"\\python3 ~/EPFL/inscape_scripts/main.py"
       # ])
...
class Lectures(list):
    ...
    def compile_master(self):
        result = subprocess.run(
            ['latexmk', '-pdf', '-interaction=nonstopmode', '-synctex=1', str(self.master_file)],
    ...
    def export_master(self):
        in_file = Path(self.root) / "master.pdf"
        out_file = os.path.expanduser("~/kDrive2/" + self.course.name + ".pdf")
        result = subprocess.run(
            ['cp', in_file, out_file],
            #stdout=subprocess.DEVNULL,
            #stderr=subprocess.DEVNULL,
